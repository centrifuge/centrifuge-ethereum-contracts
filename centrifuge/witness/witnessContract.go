...
type EthereumWitness struct {
	...
	EthereumWitnessFilterer   // Log filterer for contract events
}
...
// EthereumWitnessFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type EthereumWitnessFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}
...
func NewEthereumWitness(address common.Address, backend bind.ContractBackend) (*EthereumWitness, error) {
	contract, err := bindEthereumWitness(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &EthereumWitness{EthereumWitnessCaller: EthereumWitnessCaller{contract: contract}, EthereumWitnessTransactor: EthereumWitnessTransactor{contract: contract}, EthereumWitnessFilterer: EthereumWitnessFilterer{contract: contract}}, nil
}
...
func NewEthereumWitnessCaller(address common.Address, caller bind.ContractCaller) (*EthereumWitnessCaller, error) {
	contract, err := bindEthereumWitness(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
...
func NewEthereumWitnessTransactor(address common.Address, transactor bind.ContractTransactor) (*EthereumWitnessTransactor, error) {
	contract, err := bindEthereumWitness(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
...
// NewEthereumWitnessFilterer creates a new log filterer instance of EthereumWitness, bound to a specific deployed contract.
func NewEthereumWitnessFilterer(address common.Address, filterer bind.ContractFilterer) (*EthereumWitnessFilterer, error) {
	contract, err := bindEthereumWitness(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &EthereumWitnessFilterer{contract: contract}, nil
}

// bindEthereumWitness binds a generic wrapper to an already deployed contract.
func bindEthereumWitness(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
